...
approach, 5.0 rad/s in place). 0.03 m/s is the slowest the cube can drive:
...
(`v_angular_min_in_place`, 2.5 rad/s for Graceful) are kept above that.
...
| final yaw error, mean / max | 20° / 26° | **10° / 12°** |
...
gone (at most 6 mm off the path at the zigzag corners). Graceful lands on the
final heading more accurately because its final rotation runs at
`v_angular_min_in_place` (2.5 rad/s) instead of RPP's 5.0 rad/s, which
overshoots by one or two 20 Hz steps (see below). RPP stays the default.

Both controllers end 10-26° off the goal heading while `yaw_goal_tolerance`
is 0.1 rad: at 5 rad/s an in-place turn covers 14° per control step and about
17° per 60 ms of BLE latency, so the goal checker succeeds the moment the
heading passes through the band and the cube overshoots afterwards. A slower
in-place turn (or a wider yaw tolerance) is needed to actually settle on the
heading; that is left for a follow-up.
